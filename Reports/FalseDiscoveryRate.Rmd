...
As a false negative is far more worrisome than a false positive, and hence diagnostic tests are often very sensitive. You rather raise a false alarm than miss an infection. But a higher sensitivity also increases the probability of a false positive, and those are said to inflate the numbers. But is that really true? Let's take a look at the (very) basic theory behind test performance. 
...
Diagnostic tests all have a certain sensitivity and specificity. These are properties of the test itself (assuming they're carried out correctly and consistently), so they aren't influenced by who you test or how many infections there really are in your population.

## The False Discovery Rate

The false discovery rate or $FDR$ is simply the proportion of false positives on the total of your positive test results. This number is influenced by both the sensitivity and specificity, so we need to know how exactly.

Let's assume for a moment we know the prevalence. Then the proportion of true positives in your test results is calculated as $P \times Se$, i.e. the proportion of real positives times the "detection rate" or sensitivity. Likewise, false positives over the entire test population can be calculated as $(1 - P) \times (1 - Sp)$. Here is $(1 - P)$ is the proportion of true negatives, and from that group a proportion of $(1 - Sp)$ will test positive even though they're not ill. 
...
With this information, you can write down a formula to estimate the false discovery rate $FDR$. Remember, this is nothing else but the proportion of false positives on the total of positive tests :

$$
FDR = \displaystyle\frac{(1 - P) \times (1 - Sp)}{N}
$$

To really know the FDR, you need to know how sensitive and specific your test is, and also how much people are really infected. But we don't know the latter, we can only see the proportion of positive tests $N$. Luckily you can rewrite the equation for $N$ above in such a way that you can use the specificity, sensitivity and proportion of positive tests to estimate the prevalence:
...
It's crucial to know what the sensitivity and specificity are for your tests, but in the case of COVID this isn't very straightforward. PCR methods are in general very specific and rather sensitive. That's also the reason they're used so often for detection of viruses. In the literature you find numbers for specificity ranging from 97% to 100%, and for sensitivity from 66% to 95%. This depends partly on the criteria used for testing, and the exact flavor of the PCR procedure.

Keep in mind it is impossible to have a false positive rate that's larger than the proportion of positive tests. You can't claim that 2% of your negative population tests positive if you only have 0.5% positive tests over the total of tests. Even if there are no infections, a specificity of 98% means that 2% of your healthy population will test positive regardless. As Belgium reported positive rates as low as 0.5%, it's not unreasonable to assume the specificity of the tests carried out here reaches a whopping 99.5%. We're good at this!
