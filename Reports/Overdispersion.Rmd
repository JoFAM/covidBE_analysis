...
psim <- simul_prob_containment(
  nstart = c(1,10,30,50,100,300,500,1000, 3000,5000,10000),
  nsimul = 100, fold = 10,
  nstep = 10,
  k = 0.2, R = 1.3
)
...
psim$lim <- "no limit"
psimlim5$lim <- "max 5 missed"
psimlim3$lim <- "max 3 missed"
psimlim1$lim <- "max 1 missed"

pdata <- rbind(psim, psimlim3, psimlim5,psimlim1)
...
ggplot(pdata, aes(x=nstart,y=p, color = lim)) + 
  ...
  scale_x_log10() +
  scale_y_continuous(labels = label_percent()) +
  labs(x = "Initial infections",
       y = "probability containment",
